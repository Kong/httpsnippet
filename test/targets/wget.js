...
describe('Wget', function () {
  it('should convert simple request to Wget command', function (done) {
    ...
    result.should.eql('wget --quiet --method POST --header "Cookie: bar=baz" --header "Content-Type: application/json" --body-data "{\\"foo\\": \\"bar\\"}" --output-document - "http://mockbin.com/request?foo=bar"');
  ...
  it('should ask for verbose output', function (done) {
    ...
    result.should.be.a.String;
    result.should.eql('wget -v --method POST --header "Cookie: bar=baz" --header "Content-Type: application/json" --body-data "{\\"foo\\": \\"bar\\"}" -O - "http://mockbin.com/request?foo=bar"');
