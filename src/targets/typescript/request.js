/**
 * @description
 * HTTP code snippet generator for Node.js using Request with Typescript types built in.
 *
 * @author
 * @obfuscation
 *
...
module.exports = function (source, options) {
  ...
  code.push('import * as request from "request";')
      .blank()
  ...
  if (source.cookies.length) {
    ...
    code.push('let jar: request.CookieJar = request.jar();')
  ...
  if (includeFS) {
    code.unshift('let fs = require("fs");')
  }

  code.push('let options: request.CoreOptions = %s;', stringifyObject(reqOpts, { indent: '  ', inlineCharacterLimit: 80 }))
    .blank()

  code.push(util.format('request(options, %s', '(error: Error, response: request.Response, body: string) => {'))

      .push(1, 'if (error) throw error;')
      .blank()
