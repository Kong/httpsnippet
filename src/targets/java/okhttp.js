...
module.exports = function (source, options) {
  ...
  if (source.postData.text) {
    if (source.postData.boundary) {
      code.push(`MediaType mediaType = MediaType.parse("${source.postData.mimeType}; boundary=${source.postData.boundary}");`);
    } else {
      code.push(`MediaType mediaType = MediaType.parse("${source.postData.mimeType}");`);
    }
    code.push(`RequestBody body = RequestBody.create(mediaType, ${JSON.stringify(source.postData.text)});`);
  }
  ...
  code.push('.build();').blank().push('Response response = client.newCall(request).execute();', 1);
